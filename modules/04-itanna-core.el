...
(defun itanna/open-notebook ()
  "Create a new notebook file in ~/.itanna/notebooks/ from the template.

Prompts for a file name, copies the org-notebook.org template to
~/.itanna/notebooks/NAME.org, and opens it for editing.
This keeps user notebooks separate from the distribution."
  (interactive)
  (let* ((notebooks-dir (expand-file-name "notebooks" (expand-file-name "~/.itanna/")))
         (template (expand-file-name "templates/org-notebook.org" itanna-root))
         (default-name (format-time-string "notebook-%Y%m%d"))
         (name (read-string (format "Notebook name [%s]: " default-name)
                            nil nil default-name))
         (filepath (expand-file-name (concat name ".org") notebooks-dir)))
    ;; Create notebooks directory if needed
    (unless (file-directory-p notebooks-dir)
    ...
    (find-file filepath)
    (when (fboundp 'itanna/ensure-venv)
      (itanna/ensure-venv))))
...
This erases and regenerates the buffer each time, so calling it
from any buffer via `; h' always shows a fresh welcome page."
  (interactive)
  (itanna/ensure-venv)
  (let* ((buf (get-buffer-create itanna-welcome-buffer))
    ...
    (with-current-buffer buf
      ...
      (insert "#+OPTIONS: ^:nil\n")
      (insert "\n")
      ;; Button bar — each is a clickable elisp link
    ...
    ;; Switch to the buffer (or display it)
